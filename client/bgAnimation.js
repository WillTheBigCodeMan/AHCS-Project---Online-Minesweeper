...
for (let i = 0; i < 50; i++) {
  circles.push({
    baseColour: "",
    x: Math.random() * bgcvs.width - bgcvs.width / 2,
    y: Math.random() * bgcvs.height - bgcvs.height / 2,
...
setInterval(function () {
  ...
  for (let i = 0; i < circles.length; i++) {
    if (circles[i].c != circles[i].baseColour) {
      ...
      let newC =
        "#" + redC.toString(16) + greenC.toString(16) + blueC.toString(16);
      circles[i].c = newC;
    }
    bgctx.fillStyle = circles[i].c;
    bgctx.beginPath();
    bgctx.arc(
      circles[i].x + bgcvs.width / 2,
      circles[i].y + bgcvs.height / 2,
      circles[i].width, 0, Math.PI * 2 );
    bgctx.fill();
    ...
    for (let j = 0; j < circles.length; j++) {
      if ( circles[i].cooldown < 0 && circles[j].cooldown < 0 && j !== i &&
        Math.sqrt(Math.pow(circles[i].x - circles[j].x, 2) + Math.pow(circles[i].y - circles[j].y, 2) ) < circles[i].width + circles[j].width) {
